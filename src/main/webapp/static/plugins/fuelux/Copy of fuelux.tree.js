...
(function (factory) {
	...
	Tree.prototype = {
		...
		selectItem: function (el) {
			...
			if($el.hasClass('tree-selected')) {
				...
				if($icon.hasClass(this.options['selected-icon']) || $icon.hasClass(this.options['unselected-icon']) ) {
					$icon.removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);//
				}
			} else {
				$el.addClass ('tree-selected');
				// add tree dot back in
				if($icon.hasClass(this.options['selected-icon']) || $icon.hasClass(this.options['unselected-icon']) ) {
					$icon.removeClass(this.options['unselected-icon']).addClass(this.options['selected-icon']);//
				}
				if (this.options.multiSelect) {
					data.push( selData );
				}
		...
		openFolder: function (el) {
			...
			if ($el.find('.'+$.trim(this.options['close-icon'].replace(/(\s+)/g, '.'))).length) {//
				eventType = 'opened';
		...
		collapse: function () {
			...
			// find open folders
			this.$element.find('.'+$.trim(this.options['open-icon'].replace(/(\s+)/g, '.'))).each(function () {//
				// update icon class
				var $this = $(this)
