...
(function (factory) {
	...
	var Tree = function Tree(element, options) {
		...
		if (this.options.folderSelect) {
			...
			this.$element.on('click.fu.tree', '.icon-folder', $.proxy(function (ev) {
	...
	Tree.prototype = {
		...
		discloseFolder: function discloseFolder(el) {
			...
			$branch.find('> .tree-branch-header .icon-folder').eq(0)
				.removeClass('yunmel-icon tree-plus')
				.addClass('yunmel-icon tree-minus');
		...
		closeFolder: function closeFolder(el) {
			...
			$branch.find('> .tree-branch-header .icon-folder').eq(0)
				.removeClass('yunmel-icon tree-minus')
				.addClass('yunmel-icon tree-plus');
		...
		toggleFolder: function toggleFolder(el) {
			var $el = $(el);
			if ($el.find('.tree-plus').length) {
				this.discloseFolder(el);
			} else if ($el.find('.tree-minus').length) {
				this.closeFolder(el);
			}
		},
	...
	function styleNodeSelected ($element, $icon) {
		...
		if ($element.hasClass('tree-item') && $icon.hasClass('fa-times') ) {
			$icon.removeClass('fa-times').addClass('fa-check'); // make checkmark
		}
	...
	function styleNodeDeselected ($element, $icon) {
		...
		if ( $element.hasClass('tree-item') && $icon.hasClass('fa-check') ) {
			$icon.removeClass('fa-check').addClass('fa-times'); // make bullet
		}
	}
